export const todoApi = {
    todos: [
        ...
        {content: 'watch some code', isCompleted: false},
        {content: 'listen some code', isCompleted: false},
    ],
    ...
    delTodo: async function(index) {
        return new Promise((resolve, reject) => {
            setTimeout(()=> {
                if (!Number.isInteger || index < 0 || index >= this.todos.length) {
                    reject({error: 'index is not valid in del'});
                    return;
                }
                this.todos = [
                ...
                resolve({delTodo : 'succeed in del.'});
